...
addClass cls mdoc allClasses = do
  ...
  let classDecls =
        nub $
          (noComments <$> (mkConstants cls ++ mkEnums cls))
            ++ signals
            ++ properties
            ++ methods
  modules
    %= HM.insert
      ...
      ( HS.Module
          ...
          [ HS.LanguagePragma
              Nothing
              [ HS.Ident Nothing "DerivingStrategies",
  ...
  where
    classImports =
      map
        ...
        ( [ "Data.Coerce",
            ...
            "Linear(V2(..),V3(..),M22)",
            "Data.Colour(withOpacity)",
            "Data.Colour.SRGB(sRGB)",
            "System.IO.Unsafe",
    ...
    parentModuleImport = case (cls ^. baseClass, V.find (\x -> cls ^. baseClass == x ^. name) allClasses) of
      ...
      (_, Just baseCls) ->
        Just $
          "Godot."
            ++ (pascal $ T.unpack (baseCls ^. apiType))
            ++ "."
            ++ (T.unpack $ mangleClass $ baseCls ^. name)
            ++ "()"
    classExports decls =
      HS.ExportSpecList Nothing $
        mapMaybe
          ( \decl -> case decl of
...
mkProperties cls mdoc = concat <$> mapM mkProperty (V.toList $ cls ^. properties)
  where
    ...
    indexArg prop =
      if prop ^. index == -1
        then V.empty
        else V.singleton (GodotArgument "param" (PrimitiveType IntType) Nothing)
    mkGetter prop =
      mkMethod
        cls
        ( GodotMethod
    ...
    mkProp prop = do
      ...
      where
        ...
        ro =
          if T.null (prop ^. setter)
            then HS.TyPromoted () $ HS.PromotedCon () True $ HS.UnQual () $ HS.Ident () "True"
            else HS.TyPromoted () $ HS.PromotedCon () True $ HS.UnQual () $ HS.Ident () "False"
        ...
        get =
          if prop ^. index == -1
            then [hs|__getname__|]
            else [hs|wrapIndexedGetter __i__ __getname__|]
        set =
          if prop ^. index == -1 || T.null (prop ^. setter)
            then
              if T.null (prop ^. setter)
                then [hs|()|]
                else [hs|wrapDroppingSetter __setname__|]
            else [hs|wrapIndexedSetter __i__ __setname__|]
...
mkMethods cls mdoc =
  concat
    <$> mapM
      (\m -> mkMethod cls m (methodDoc (m ^. name) mdoc))
  ...
  where
    ...
    methodArgs =
      let args = fmap argToHsType (method ^. arguments)
       in if method ^. hasVarargs
            then args `V.snoc` [ty|[Variant 'GodotTy]|]
            else args
